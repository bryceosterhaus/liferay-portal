...
```bash
yarn compile
# Used to create the static icon spritemap
yarn buildIcons
yarn build
```
...
-   `@clayui/shared` is primarily an internal package for shared utilities used between ClayUI components and should not be treated as a primary end-user entry point.
...
-   Published npm packages are primarily for external consumers and type/package distribution.
...
-   Deploying only one side can lead to incomplete or misleading style results.
...
-   Keep component props local to `IProps`, and type component params directly as `({ ... }: IProps)` so API docs tooling can consume the type.
...
-   Set `displayName` explicitly so React DevTools shows the intended component name.
