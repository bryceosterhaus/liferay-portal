...
export class VoiceNoteComponent implements  AfterViewInit {
  ...
  constructor(private audioRecorderService: NgAudioRecorderService,private renderer: Renderer2,private sanitizer: DomSanitizer) {
  }
  // @ts-ignore
  ...
  stop()
  {
    let prom = new Promise((resolve, reject)=>{
      this.audioRecorderService.stopRecording(this.outputFormat).then((output) => {
        resolve(this.getAudioToFile(output as Blob,"audioNote.webm"));
      }).catch(errorCase => {
        reject(errorCase)
      });
    });
  ...
  async stopRecording()
  {
    ...
    console.log("called")
    this.rawAudioObject ='data:audio/webm;base64,' + ( await this.blobToBase64(file as Blob));
